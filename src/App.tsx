...
import { 
  ...
  CheckCircle2, ArrowUpRight, ArrowDownLeft, Zap, X, QrCode, Clock, ExternalLink 
} from 'lucide-react';
...
// -- UPDATED MODAL (With the Processing State)
      ...
      <div className="bg-white w-full max-w-xs sm:max-w-sm rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            <button onClick={rejectTx} disabled={isProcessingTx} className="py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100 transition-colors disabled:opacity-50">
              Reject
            </button>
            <button 
              onClick={confirmTx} 
              disabled={isProcessingTx}
              className="bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-bold shadow-lg shadow-indigo-200 transition-all active:scale-95 flex justify-center items-center gap-2"
            >
              {isProcessingTx ? <Loader2 className="animate-spin" size={18}/> : "Confirm"}
            </button>
          </div>
        </div>
      </div>
...
const DashboardView = () => {
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // UPDATE UI (This runs only if TRx succeeds)
      setTxHash(signature);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex justify-center font-sans">
      <div className="w-full max-w-md bg-white min-h-screen flex flex-col shadow-2xl relative">
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          ...
          {txHash && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-2xl flex items-start gap-3 animate-in fade-in slide-in-from-bottom-2 mb-6">
